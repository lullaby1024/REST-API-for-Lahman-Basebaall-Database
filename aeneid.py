...
def compute_links(result, limit, offset):
    """
    Computes the links for given limit and offset
    :param result
    :param limit
    :param offset
    :return: a dictionary with 'data' and 'links'
    """
    ...
    if offset:
        if limit:
            next_offset = int(offset) + int(limit)  # 0, 10, ...

            base = request.base_url
            args = {}
            for k,v in request.args.items():
                if not k == 'offset':  # if the offset does not change
                    args[k] = v
                else:
                    args[k] = next_offset

            params = urlencode(args)  # convert dictionary into url
            self = {"rel": "next", "href": base + "?" + params}  # link to next page
            result['links'].append(self)
...
def handle_resource(dbname, resource_name, primary_key):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if result:
                resp = Response("UPDATED", status=200, mimetype="text/plain")
            else:
                resp = Response("NOT FOUND.", status=404, mimetype="text/plain")
...
def handle_collection(dbname, resource_name):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            field_list = request.args.get('fields', None)  # look for 'fields' and return None if it DNE
            if field_list is not None:
                field_list = field_list.split(",")
            ...
            # Find by template.
            result = ds.get_by_template(resource, children, tmp, field_list=field_list,
...
@app.route('/api/<dbname>/<resource_name>/<primary_key>/<related_resource_name>', methods=['GET','POST'])
def handle_related(dbname, resource_name, primary_key, related_resource_name):
    ...
    try:
        ...
        # Form the compound resource names dbschema.table_name
        resource = dbname + "." + resource_name
        related_resource = dbname + "." + related_resource_name
        ...
        if request.method == 'GET':
            ...
            result = ds.get_related(resource, key_columns, related_resource, tmp,
                                    field_list=field_list, limit=limit, offset=offset, order_by=order_by)
            ...
            new_r = request.get_json()
            result = ds.insert_by_path(resource, key_columns, related_resource, new_r)
